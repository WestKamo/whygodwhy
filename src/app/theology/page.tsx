...
// --- SCRIPTURE TOOLTIP COMPONENT (Unchanged) ---
...
// --- SAFE MANUSCRIPT PARSER (Unchanged) ---
        ...
        return (
          <div key={i} style={{ marginBottom: '1.5em', display: 'block' }}>
            {parts.map((part, j) => {
              ...
              if (isVerse) return <ScriptureTooltip key={j} reference={part} />;
...
export default function TheologyPage() {
  ...
  return (
    <div style={{ backgroundColor: '#fdfcf8', minHeight: '100vh', position: 'relative' }}>
      ...
      {/* --- SCROLLABLE EDITOR (ENHANCED) --- */}
      <AnimatePresence>
        {showEditor && isAdmin && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={masterOverlayStyle}>
            <motion.div initial={{ scale: 0.95, y: 50 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 50 }} style={editorBubbleStyle}>
              <div style={{ padding: '40px 60px 20px', borderBottom: '1px solid #f5f5f0' }}>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '30px 60px' }}>
                ...
                <textarea rows={12} value={form.content} onChange={e => setForm({...form, content: e.target.value})} style={{...inputStyle, resize: 'none'}} placeholder="Begin the inquiry..." />
                
                <label style={labelStyle}>SOURCES & CITATIONS</label>
                <textarea rows={4} value={form.sources} onChange={e => setForm({...form, sources: e.target.value})} style={{ ...inputStyle, borderBottom: 'none', resize: 'none' }} placeholder="St. Augustine, Confessions..." />
              </div>
              
              <div style={footerStyle}>
                <button onClick={() => setShowEditor(false)} style={{ ...publishBtnStyle, background: 'none', color: '#aaa', border: 'none' }}>DISCARD</button>
      ...
      {/* --- FULL-FLOW READ BUBBLE --- */}
      <AnimatePresence>
        {activeArticle && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={readOverlayStyle}>
            <motion.div initial={{ scale: 0.95, y: 30 }} animate={{ scale: 1, y: 0 }} style={readScrollCard}>
              <header style={{ textAlign: 'center', marginBottom: '60px' }}>
                <h1 style={{ fontFamily: 'serif', fontSize: 'clamp(32px, 8vw, 64px)', color: '#111', marginBottom: '15px' }}>{activeArticle.title}</h1>
                <p style={{ fontSize: '10px', letterSpacing: '4px', color: '#b39359', fontWeight: 'bold' }}>
                  A TREATISE BY P. SANDI — {activeArticle.category.toUpperCase()}
                </p>
              </header>
              
              <div style={{ fontSize: 'clamp(18px, 4vw, 21px)', lineHeight: '2.1', fontFamily: 'serif', color: '#333' }}>
                <ProcessedContent text={activeArticle.content} />
              </div>

              {activeArticle.sources && (
                <div style={{ marginTop: '80px', padding: '40px', borderTop: '1px solid #f0f0eb' }}>
                  <span style={labelStyle}>SOURCES</span>
                  <p style={{ fontSize: '14px', fontStyle: 'italic', color: '#888' }}>{activeArticle.sources}</p>
                </div>
              )}
        ...
        {showValidationAlert && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={alertOverlayStyle}>
            <div style={alertCardStyle}>
              ...
              <p style={{ fontSize: '14px', color: '#666' }}>Title and content are required.</p>
...
const readOverlayStyle: React.CSSProperties = { position: 'fixed', inset: 0, backgroundColor: '#fdfcf8', zIndex: 1000000, display: 'flex', justifyContent: 'center', alignItems: 'flex-start', overflowY: 'auto', padding: '0' };

const readScrollCard: React.CSSProperties = { maxWidth: '950px', width: '100%', minHeight: '100vh', padding: '160px 40px 100px', backgroundColor: '#fdfcf8', position: 'relative' };

const editorBubbleStyle: React.CSSProperties = { backgroundColor: '#fff', width: '100%', maxWidth: '850px', borderRadius: '40px', boxShadow: '0 50px 100px rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column', maxHeight: '85vh', border: '1px solid rgba(179, 147, 89, 0.1)', overflow: 'hidden' };

const footerStyle: React.CSSProperties = { padding: '20px 60px 40px', display: 'flex', gap: '20px', borderTop: '1px solid #f5f5f0', backgroundColor: '#fff', position: 'sticky', bottom: 0 };
...
const alertCardStyle: React.CSSProperties = { backgroundColor: '#fff', padding: '40px', borderRadius: '30px', maxWidth: '380px', textAlign: 'center', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' };
...
const actionBtn: React.CSSProperties = { flex: 1, padding: '15px', border: 'none', borderRadius: '40px', color: '#fff', fontWeight: 'bold', cursor: 'pointer', fontSize: '10px' };
const labelStyle: React.CSSProperties = { display: 'block', fontSize: '9px', fontWeight: 'bold', letterSpacing: '2px', color: '#b39359', marginBottom: '10px', marginTop: '20px' };
...
const publishBtnStyle: React.CSSProperties = { flex: 2, padding: '18px', background: '#111', color: '#fff', border: 'none', borderRadius: '50px', fontWeight: 'bold', letterSpacing: '2px', fontSize: '10px', cursor: 'pointer' };
const closeReadBtn = { background: '#111', color: '#fff', padding: '15px 40px', border: 'none', borderRadius: '50px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', letterSpacing: '2px' };
